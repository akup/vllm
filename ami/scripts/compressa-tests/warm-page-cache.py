...
def read_file_into_page_cache(path: Path, chunk_size: int) -> tuple[Path, int]:
...
def main() -> int:
    ...
    print(f"Warming page cache: {len(files)} files, {total_gib:.1f} GiB, {args.workers} workers")
    start = time.perf_counter()
    last_log_time = start
    last_log_gib = 0.0
    log_interval_gib = 1.0  # log every 1 GiB
    log_interval_sec = 5.0  # and at least every 5 seconds
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = {
            executor.submit(read_file_into_page_cache, p, args.chunk_size): p
            for p in files
        }
        for future in as_completed(futures):
            ...
            if n >= 0:
                ...
                elapsed = now - start
                if (gib_read - last_log_gib >= log_interval_gib) or (now - last_log_time >= log_interval_sec):
                    pct = 100.0 * bytes_read / total_size if total_size else 0
                    mibs = (bytes_read / (1024**2)) / elapsed if elapsed > 0 else 0
                    print(f"  Progress: {gib_read:.1f} GiB read and cached ({pct:.0f}%), {mibs:.0f} MiB/s, elapsed {elapsed:.0f}s")
    ...
    print(f"Done: {gib_read:.1f} GiB read and cached in {elapsed:.1f}s total ({throughput_mibs:.0f} MiB/s), {read_count} files read, {error_count} errors")
